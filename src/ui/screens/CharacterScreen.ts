  ...
  setCharacter(character: Character): void {
    ...
    if (character.spellcasting) {
      ...
      if (sc.cantripsKnown.length > 0) {
        ...
        spellSection.appendChild(el('div', { class: 'char-spell-list' }, [sc.cantripsKnown.join(', ')]));
      }
      if (sc.preparedSpells.length > 0) {
        spellSection.appendChild(el('div', { class: 'char-spell-list-label' }, ['Prepared Spells']));
        spellSection.appendChild(el('div', { class: 'char-spell-list' }, [sc.preparedSpells.join(', ')]));
      }
